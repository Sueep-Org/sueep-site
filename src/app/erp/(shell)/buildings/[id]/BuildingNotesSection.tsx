...
export function BuildingNotesSection({
  buildingId,
  initialNotes,
  currentUserId,
}: {
  ...
  async function postNote(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/erp/buildings/${buildingId}/notes`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ body: draft.trim() }),
      });
      const data = (await res.json()) as BuildingNoteRow & { error?: string };
      if (!res.ok) {
  ...
  function startEdit(note: BuildingNoteRow) {
  ...
  async function saveEdit(noteId: string) {
    ...
    try {
      const res = await fetch(`/api/erp/buildings/${buildingId}/notes/${noteId}`, {
        method: "PATCH",
        headers: { "content-type": "application/json" },
  ...
  async function deleteNote(noteId: string) {
    ...
    try {
      const res = await fetch(`/api/erp/buildings/${buildingId}/notes/${noteId}`, { method: "DELETE" });
