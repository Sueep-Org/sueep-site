...
export function ChangeOrderMaterialsSection({
  projectId,
  changeOrderId,
  initialEntries,
  onTotalChange,
  ...
  useEffect(() => { onTotalChange?.(totalCostCents); }, [totalCostCents, onTotalChange]);
  ...
  async function onAdd(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(
        `/api/erp/projects/${projectId}/change-orders/${changeOrderId}/materials`,
        {
          method: "POST",
      ...
      const data = (await res.json()) as CoMaterialRow & { error?: string };
  ...
  function startEdit(r: CoMaterialRow) {
    setEditingId(r.id);
    setEditFields({
  ...
  async function onSaveEdit(id: string) {
    const res = await fetch(
      `/api/erp/projects/${projectId}/change-orders/${changeOrderId}/materials/${id}`,
      {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
    ...
    if (res.ok) {
      const updated = (await res.json()) as CoMaterialRow;
    ...
    <div className="space-y-6">
      <form onSubmit={onAdd} className="rounded-lg border border-gray-200 bg-gray-50 p-4">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div>
            <label className={label} htmlFor="com-usedOn">Date *</label>
            <input id="com-usedOn" name="usedOn" type="date" required className={input} />
          </div>
          <div>
            <label className={label} htmlFor="com-category">Category *</label>
            <select id="com-category" name="category" defaultValue="CLEANING_PRODUCTS" className={input}>
              <option value="CLEANING_PRODUCTS">Cleaning products</option>
              <option value="PAINT">Paint</option>
            </select>
          </div>
          <div>
            <label className={label} htmlFor="com-item">Item *</label>
            <input id="com-item" name="itemName" required className={input} placeholder="e.g. Degreaser, primer" />
          </div>
          <div>
            <label className={label} htmlFor="com-qty">Quantity</label>
            <input id="com-qty" name="quantity" type="number" min={0} step={0.01} className={input} />
          </div>
          <div>
            <label className={label} htmlFor="com-unit">Unit</label>
            <input id="com-unit" name="unit" className={input} placeholder="gal, bottle, case" />
          </div>
          <div>
            <label className={label} htmlFor="com-cost">Cost (USD) *</label>
            <input id="com-cost" name="cost" required className={input} placeholder="0.00" />
          </div>
          <div className="sm:col-span-2 lg:col-span-3">
            <label className={label} htmlFor="com-notes">Notes</label>
            <input id="com-notes" name="notes" className={input} />
          </div>
        </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-3">
          <div className="flex-1 min-w-[140px]">
            <label className={label} htmlFor="comf-date">Filter by date</label>
            <input id="comf-date" type="date" className={input} value={filterDate} onChange={(e) => setFilterDate(e.target.value)} />
          </div>
          <div className="flex-1 min-w-[160px]">
            <label className={label} htmlFor="comf-item">Filter by item</label>
            <input id="comf-item" type="text" className={input} placeholder="Item name…" value={filterItem} onChange={(e) => setFilterItem(e.target.value)} />
          </div>
          {(filterDate || filterItem) && (
