...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const entry = await prisma.materialEntry.findFirst({
  ...
  const updated = await prisma.materialEntry.update({
    where: { id: materialId },
    data,
  });
  ...
  return NextResponse.json({
    ...
    category: updated.category as "CLEANING_PRODUCTS" | "PAINT",
...
export async function DELETE(_req: Request, ctx: Ctx) {
  ...
  await prisma.materialEntry.delete({ where: { id: materialId } });
