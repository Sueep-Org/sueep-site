...
export async function POST(req: Request, ctx: Ctx) {
  ...
  const project = await prisma.project.findUnique({ where: { id }, select: { id: true, jobTitle: true } });
  ...
  if (fd.get("presigned") === "true") {
    ...
    const contract = await prisma.projectContract.create({
      data: { projectId: id, contractPdfFilename: file.name, signingStatus: "SIGNED", signedAt, signedDocumentUrl: dataUrl },
    });
