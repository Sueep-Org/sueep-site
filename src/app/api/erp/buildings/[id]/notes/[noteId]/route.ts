...
async function requireOwnedNote(noteId: string, uid: string) {
  const note = await prisma.buildingNote.findUnique({ where: { id: noteId }, select: { id: true, authorUserId: true } });
...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const updated = await prisma.buildingNote.update({ where: { id: noteId }, data: { body: noteBody } });
...
export async function DELETE(_req: Request, ctx: Ctx) {
  ...
  await prisma.buildingNote.delete({ where: { id: noteId } });
