...
export async function GET(_req: Request, ctx: Ctx) {
  ...
  const assignment = await prisma.changeOrderContractorAssignment.findUnique({
    where: { id },
    include: {
      contractor: { select: { id: true, name: true } },
      changeOrder: { select: { id: true, title: true, projectId: true } },
    },
  });
...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const existing = await prisma.changeOrderContractorAssignment.findUnique({ where: { id } });
  ...
  try {
    const assignment = await prisma.changeOrderContractorAssignment.update({ where: { id }, data: data as object });
    return NextResponse.json(assignment);
  } catch (e) {
    console.error("PATCH /api/erp/change-order-contractor-assignments/[id]", e);
...
export async function DELETE(_req: Request, ctx: Ctx) {
  const { id } = await ctx.params;
  try {
    await prisma.changeOrderContractorAssignment.delete({ where: { id } });
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error("DELETE /api/erp/change-order-contractor-assignments/[id]", e);
