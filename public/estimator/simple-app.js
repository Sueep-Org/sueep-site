...
async function initApp(){
  ...
  if (downloadPdfBtn) {
    ...
    downloadPdfBtn.addEventListener('click', exportPdfWithAnnotations);
  }
  ...
  async function exportPdfWithAnnotations(){
    ...
    try {
      ...
      if (!pageCount) {
        toast('No pages available to export.', 'error');
        return;
      }
      ...
      for (let pageNumber = 1; pageNumber <= pageCount; pageNumber += 1) {
        ...
        overlay.setCurrentPage(pageNumber);
        overlay.redraw();
        ...
        pageImages.push(exportCanvas.toDataURL('image/png'));
      }
      ...
      const pageMarkup = pageImages.map((imageUrl, index) => `
        <img src="${imageUrl}" alt="Exported PDF page ${index + 1}" style="display:block;width:100%;height:auto;page-break-after:always;" />
      `).join('');
      ...
      console.error(error);
      toast('Unable to export the PDF with annotations.', 'error');
    }
  }

  function isAreaMeasurementRecord(measurement) {
    return !!measurement && (
  ...
  function updateMeasurementList(){
    ...
    const lineMeasurements = measurements.filter(m => !isAreaMeasurementRecord(m));
    const areaMeasurements = measurements.filter(isAreaMeasurementRecord);
      ...
      if (!lineMeasurements.length) {
        ...
        measurementListLeft.querySelectorAll('button[data-measurement-id]').forEach((btn) => {
          btn.onclick = () => {
            ...
            if (!id) return;
            const removed = highlightsStore.removeMeasurement(measurementViewPage, id);
            if (removed) {
              overlay.setCurrentPage(measurementViewPage);
