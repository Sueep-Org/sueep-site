...
function standardizeScaleExpression(expression = '') {
  ...
  const inchesPerDrawingInch = rightInches / leftInches;
  const denominator = formatStandardScaleDenominator(inchesPerDrawingInch);
  return denominator ? `${denominator} in = 1 ft` : null;
}
const examples = ['1/8" = 1\'', '1/8" = 1\'-0"', '1/8" = 1\' 0"', '1/8" = 1 ft', '1/8" = 1\' - 0"'];
examples.forEach(expr => {
  console.log(JSON.stringify(expr), '=>', standardizeScaleExpression(expr));
});
